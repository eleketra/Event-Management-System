...
public class Event_listforClub extends JPanel {
	...
	/*public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Event_listforClub window = new Event_listforClub();
	 ...
	 */
	public Event_listforClub(String str) {
		initialize(str);
	}
	...
	private void initialize(String str) {
		...
		JLabel lblNewLabel = new JLabel("Organized Events");
		...
		textArea.setText("EVENT NAME"+"\t\t"+"   DATE"+"\t"+"DURATION"+"\t"+"VENUE"+"\t"+"DECRIPTION"+"\n\n");
		try{
			...
			Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/event_management","ankita","ankita");
				...
				while(rst.next())
				{
					if(rst.getString("clubname").equals(str))
					{
